...
-define(DEFAULT_COUNT, 6). %% seconds
...
init([Opts]) ->
    timer(timer:seconds(?DEFAULT_INTERVAL), collect),
    timer(get_today_remaining_seconds(), clear_expire_data),
    ExpireInterval = application:get_env(emqx_dashboard, expire_interval, ?EXPIRE_INTERVAL),
    {ok, #{count => maps:get(count, Opts, ?DEFAULT_COUNT),
           collect => ?COLLECT,
...
handle_info(collect, State = #{collect := Collect, count := 1, temp_collect := TempCollect, last_collects := LastCollect}) ->
    ...
    timer(timer:seconds(?DEFAULT_INTERVAL), collect),
    {noreply, State#{count => ?DEFAULT_COUNT,
                     collect => ?COLLECT,
...
temp_collect({_, _, Received, Sent}) ->
    ...
    Sent1 = collect(sent),
    {(Received1 - Received) div ?DEFAULT_INTERVAL,
     (Sent1 - Sent) div ?DEFAULT_INTERVAL,
     Received1,
     Sent1}.
...
flush({Connection, Route, Subscription}, {Received0, Sent0, Dropped0}) ->
    ...
    Ts = get_local_time(),
    mnesia:dirty_write(emqx_collect, #mqtt_collect{timestamp = Ts, collect = Collect}),
    {Received, Sent, Dropped}.

avg(Items) ->
    lists:sum(Items) div ?DEFAULT_COUNT.
