...
-define(MAGIC_CONFIG_NOT_FOUND, '$0tFound').
...
find([]) ->
    case do_get(?CONF, [], ?MAGIC_CONFIG_NOT_FOUND) of
...
-spec find_raw(emqx_utils_maps:config_key_path()) ->
    {ok, term()} | {not_found, emqx_utils_maps:config_key_path(), term()}.
find_raw([]) ->
    case do_get_raw([], ?MAGIC_CONFIG_NOT_FOUND) of
        ?MAGIC_CONFIG_NOT_FOUND -> {not_found, []};
        Res -> {ok, Res}
    end;
find_raw(KeyPath) ->
...
do_get(Type, KeyPath) ->
    case do_get(Type, KeyPath, ?MAGIC_CONFIG_NOT_FOUND) of
        ?MAGIC_CONFIG_NOT_FOUND -> error({config_not_found, KeyPath});
        Res -> Res
    end.
