...
inject_root_fun(Options) ->
    case proplists:get_value(ssl_options, Options) of
        undefined ->
            Options;
        SslOpts ->
            replace(Options, ssl_options, emqx_tls_lib:opt_partial_chain(SslOpts))
    end.
...
opt_partial_chain(SslOpts) ->
    case proplists:get_value(partial_chain, SslOpts, undefined) of
        undefined ->
            SslOpts;
        root_from_cacertfile ->
            replace(SslOpts, partial_chain, rootcert_from_cacertfile(SslOpts))
    end.
...
rootcert_from_cacertfile(SslOpts) ->
    ...
    emqx_const_v2:make_tls_root_fun(rootcert_from_cacertfile, CADer).
